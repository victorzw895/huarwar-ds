...
describe("Grid Component", () => {
  it("renders component with correct children text", async () => {
    render(<Grid children="sample text" data-testid="ta.grid.container" />);

    const { getByText } = within(screen.getByTestId("ta.grid.container"));
  ...
  it("renders component with correct children", async () => {
    render(
      <Grid container data-testid="ta.grid.container">
        <Grid large={6}>
          <div>6</div>
        </Grid>
        <Grid large={5}>
    ...
    const { children } = screen.getByTestId("ta.grid.container");
